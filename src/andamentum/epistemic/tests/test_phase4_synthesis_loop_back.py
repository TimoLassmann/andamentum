...
falls through to Synthesize. This means the loop-back terminates as
soon as no claim can make progress, even if the satisfaction LLM
keeps insisting more is needed.

These tests pin:
  1. needs_more + eligible claims → routes to Scrutinize, adds claims
     to rescrutiny.
  2. needs_more + all claims at per-claim cap → routes to Synthesize
     (cap-driven termination).
  3. needs_more + all claims cycle-capped → routes to Synthesize
     (terminal-state termination).
  4. needs_more + all claims abandoned → routes to Synthesize.
...
# ── all claims at per-claim cap → fall through to Synthesize ─────────


async def test_needs_more_with_all_claims_at_cap_synthesizes(
    tmp_path: Path,
) -> None:
    """The load-bearing safety: when all non-abandoned, non-cycle-
    capped claims have hit ``SCRUTINY_RESOLVE_CYCLE_CAP``, no claims
    can make progress. The node accepts the current state and routes
    to Synthesize even though demand says needs_more."""
    ...
    assert isinstance(next_node, Synthesize), (
        "When all claims have hit per-claim cap, Phase 4 must NOT "
        "loop back (would cycle forever). The cap-driven termination "
        "is the load-bearing safety; this is the test that pins it."
    )
...
async def test_needs_more_with_all_claims_cycle_capped_synthesizes(
    tmp_path: Path,
) -> None:
    """Cycle-capped claims are also terminal — same safety as the
    per-claim cap, but a different state field."""
    _obj, _claims, state, repo = await _setup_unsatisfying_run(
...
async def test_needs_more_with_all_claims_abandoned_synthesizes(
    tmp_path: Path,
) -> None:
