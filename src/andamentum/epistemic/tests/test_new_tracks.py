...
class TestContrastiveEvaluationAgent:
    ...
    def test_adapter(self):
        raw = SimpleNamespace(
            better_claim="A", distinguishing_observation="test obs", confidence=0.8
        )
    ...
    def test_adapter_normalizes_case(self):
        raw = SimpleNamespace(
            better_claim="a", distinguishing_observation="test", confidence=0.5
        )
        result = adapt_agent_output("epistemic_contrastive_evaluation", raw)
        assert result.better_claim == "A"
...
class TestContrastiveEvaluationOperation:
    ...
    async def test_sets_contrastive_checked(self, repo):
        ...
        await repo.save(obj)
        claim = Claim(
            statement="Claim A", objective_id=obj.entity_id, stage=ClaimStage.SUPPORTED
        )
...
from ..entities.evidence import Evidence as EvidenceEntity  # noqa: E402
