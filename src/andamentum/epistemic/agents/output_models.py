...
class ClarifyQuestionOutput(BaseModel):
    ...
    clarified_question: str = Field(description="Rewritten question that is unambiguous and specific")
    key_terms: list[str] = Field(description="Terms that need explicit definition for this investigation")
    reasoning: str = Field(description="Brief explanation of interpretation choice and any alternatives considered")


class ConceptualAnalysisOutput(BaseModel):
    """Output from epistemic_conceptual_analysis agent."""

    terms: list[str] = Field(description="Key terms being defined (parallel to definitions list)")
    definitions: list[str] = Field(description="Working definition for each term (parallel to terms list)")
    assumptions: list[str] = Field(description="Assumptions embedded in the question")
    context_summary: str = Field(description="2-3 sentence summary for downstream agents")
...
class AssessEvidenceOutput(BaseModel):
    ...
    confidence_estimate: float = Field(description="0.0-1.0 probability claim is true given evidence")
    justification: str = Field(description="Brief explanation of why this weight was assigned")
...
class IdentifySingleIssueOutput(BaseModel):
    ...
    has_issue: bool = Field(description="Whether there is an issue to report. Set to false if no more issues.")
    description: str = Field(description="What the issue is (empty string if has_issue is false)")
...
class DeductiveValidationOutput(BaseModel):
    ...
    deductive_soundness: str = Field(description="Overall deductive assessment - sound, questionable, or unsound")
    confidence_estimate: float = Field(description="Confidence in the deductive assessment (0.0-1.0)")
...
class VerifyComputationallyOutput(BaseModel):
    ...
    verification_code: str = Field(description="Complete Python code that tests the claim")
    packages_required: list[str] = Field(
        description="Python packages needed to run the code (e.g., numpy, scipy)"
    )
    expected_behavior: str = Field(description="What the test should output if the claim is true")
    test_description: str = Field(description="Human-readable explanation of what the test does")
...
class AnalyzeArgumentOutput(BaseModel):
    ...
    validity: str = Field(description='Does conclusion follow from premises? "valid", "invalid", or "indeterminate"')
    soundness: str = Field(description='Are premises true/supported? "sound", "unsound", or "questionable"')
...
class ResolveUncertaintyOutput(BaseModel):
    ...
    resolution: str = Field(description="How the uncertainty was resolved, if can_resolve is true")
...
class InvestigateClaimQueryItem(BaseModel):
    ...
    query: str = Field(description="Natural language search query targeting the specific gap")
...
class WriteAnswerOutput(BaseModel):
    ...
    verdict: str = Field(default="", description="One sentence answering the research question — the bottom line")
...
class AnswerValidation(BaseModel):
    ...
    approved: bool = Field(description="True if the answer faithfully represents the data, False if corrections needed")
    feedback: list[str] = Field(description="Plain text corrections describing what needs fixing. Empty if approved.")
...
class SpecifyPredictionOutput(BaseModel):
    ...
    expected_observation: str = Field(description="What should be observed if the claim is true")
...
class RecordDecisionOutput(BaseModel):
    ...
    reversible: bool = Field(description="Whether this decision can be reversed if new evidence emerges")
...
class EvaluateCounterargumentOutput(BaseModel):
    ...
    relevance: float = Field(description="0.0-1.0: Does this address the claim's specific assertions?")
    specificity: float = Field(description="0.0-1.0: Is this targeted or a general objection?")
    evidence_backed: float = Field(description="0.0-1.0: Does it cite evidence or is it speculative?")
    source_credibility: float = Field(description="0.0-1.0: Is the source authoritative for this domain?")
    category: str = Field(
...
class ClassifyEvidenceDomainOutput(BaseModel):
    ...
    method_type: str = Field(description="One of: experimental, observational, computational, theoretical")
    data_source: str = Field(description="One of: primary, secondary, synthetic, meta_analytic")
    temporal_approach: str = Field(description="One of: cross_sectional, longitudinal, retrospective, prospective")
    causal_role: str = Field(description="One of: mechanistic, phenomenological, interventional, predictive")
    confidence: float = Field(description="0.0-1.0: Overall confidence in classification")
...
class ClassifyPredictionOutput(BaseModel):
    ...
    specificity: float = Field(description="0.0-1.0: How specific and testable is this prediction?")
    ...
    justification: str = Field(description="One sentence explaining type and specificity assessment")
...
class AssessEvidenceQualityOutput(BaseModel):
    ...
    relevance: float = Field(description="0.0-1.0: How directly does this evidence address the claim?")
    specificity: float = Field(description="0.0-1.0: How specific and detailed vs. vague and general?")
    recency_appropriate: float = Field(
        description="0.0-1.0: Is this evidence current enough for the domain?"
    )
    justification: str = Field(description="One sentence explaining the quality assessment")
...
class CrossClaimConsistencyOutput(BaseModel):
    ...
    conflicts: bool = Field(
        description="Whether the two claims contradict each other"
    )
...
class FormulateQueryOutput(BaseModel):
    ...
    query: str = Field(description="Search query optimized for this provider (5-15 words)")
    rationale: str = Field(description="One sentence: why this query is appropriate for this provider")
...
class ExtractAssertionOutput(BaseModel):
    ...
    assertion: str = Field(description="One atomic factual assertion supported by this evidence (single sentence)")
...
class ScreenRelevanceOutput(BaseModel):
    ...
    is_relevant: bool = Field(description="True if the evidence contains information that helps answer the research question")
    reason: str = Field(description="One sentence explaining why this is or isn't relevant")
...
class DraftClaimOutput(BaseModel):
    ...
    statement: str = Field(description="One claim statement capturing the shared content of the assertions")
    scope: str = Field(description="Under what conditions this claim holds")
    direction: str = Field(description='"supports", "undermines", or "neutral" toward the research question')
