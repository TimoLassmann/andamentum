...
class Claim(EpistemicEntity):
    ...
    scope: str = Field(default="General", description="Under what conditions this claim applies")
    assumptions: list[str] = Field(default_factory=list, description="Explicit assumptions")
    evidence_ids: list[str] = Field(default_factory=list, description="Links to supporting evidence")
    uncertainty_ids: list[str] = Field(default_factory=list, description="Links to associated uncertainties")
    ...
    supersedes_id: Optional[str] = Field(
        default=None,
        description="ID of claim this version supersedes"
    )
    superseded_by_id: Optional[str] = Field(
        default=None,
        description="ID of claim that superseded this one"
    )
    ...
    modification_count: int = Field(
        default=0,
        description="Number of times claim has been modified"
    )
    modification_timestamps: list[str] = Field(
        default_factory=list,
        description="ISO timestamps of each modification"
    )
    ...
    adversarial_checked: bool = Field(default=False, description="Adversarial search complete")
    convergence_checked: bool = Field(default=False, description="Cross-domain convergence assessed")
    deductive_checked: bool = Field(default=False, description="Deductive validation complete")
    computational_checked: bool = Field(default=False, description="Computational verification complete (if applicable)")
    contrastive_checked: bool = Field(default=False, description="Contrastive evaluation completed")
    consistency_checked: bool = Field(default=False, description="Cross-claim consistency checked")
    routing_applied: bool = Field(default=False, description="Whether routing defaults have been applied for this claim")
    saturated: bool = Field(default=False, description="Whether investigation has stopped producing new information")

    # Denormalized for pattern matching (updated on save via model_post_init)
    evidence_count: int = Field(default=0, description="Count of evidence_ids for pattern filtering")
    ...
    needs_revalidation: bool = Field(default=False, description="Re-check current stage gate after evidence change")

    # Operation completion flags
    argument_analyzed: bool = Field(default=False, description="Argument structure analysis complete")
    predictions_generated: bool = Field(default=False, description="Testable predictions generated from claim")
    decision_recorded: bool = Field(default=False, description="Decision recorded for this actionable claim")
    ...
    def record_promotion(self, from_stage: ClaimStage, to_stage: ClaimStage, justification: str) -> None:
    ...
    @classmethod
    def from_metadata(cls, meta: dict[str, Any], statement_override: Optional[str] = None) -> "Claim":
