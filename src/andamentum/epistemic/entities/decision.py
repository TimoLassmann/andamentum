...
class Decision(EpistemicEntity):
    ...
    claim_ids: list[str] = Field(
        default_factory=list, description="Claims this decision references"
    )

    # State fields
    reversible: bool = Field(default=True, description="Whether this can be reversed")
    reversed_at: Optional[datetime] = Field(
        default=None, description="When reversed (None if active)"
    )
    reversal_reason: Optional[str] = Field(
        default=None, description="Why it was reversed"
    )
    ...
    def _from_metadata(cls, content: str, metadata: dict[str, Any]) -> "Decision":
        ...
        reversed_at = (
            datetime.fromisoformat(reversed_at_str) if reversed_at_str else None
        )
