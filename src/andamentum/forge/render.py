...
def _render_node(
    ...
    if is_head:
        ...
        if single:
            ...
            if succ == END:
                # When run_end_type is "str", use out_text (best-effort text extraction).
                # Otherwise emit the first output field directly — the spec has declared the
                # exact return type, so we use the field name directly (deterministic, no LLM).
                if run_end_type == "str":
                    tail = "        return End(out_text(out))\n"
                else:
...
def _deps_py(spec: SystemSpec, deps_name: str) -> str:
    ...
    """
    return (
        '"""Dependencies — generated by forge. Add domain handles (store, etc.) here."""\n'
        "from __future__ import annotations\n\n"
        "from dataclasses import dataclass, field\n\n\n"
        "@dataclass(frozen=True)\n"
        f"class {deps_name}:\n"
        '    """Injected, never serialized (Rule R2): model handle, test seam, caps."""\n'
        '    model: str = "test"\n'
...
def _nodes_py(
    ...
    # out_text is a best-effort helper for extracting a str from any head output.
    # Only emit it when run_end_type == "str" — when the spec declares a typed return,
    # terminal heads reference the named field directly and out_text is unused.
    out_text_block = (
        "def out_text(out: Any) -> str:\n"
        '    """Best-effort text from a head output (single-field models)."""\n'
        "    data = out.model_dump() if hasattr(out, 'model_dump') else {}\n"
        "    return next((str(v) for v in data.values() if isinstance(v, str)), str(out))\n"
        if run_end_type == "str"
        else ""
    )
    nodes = "\n\n".join(
    ...
    # Separator between out_text and the nodes section: two blank lines when out_text
    # is present (it ends with \n, so \n\n gives the standard two-blank-line gap);
    # no separator when out_text is absent (all-spine or typed-end systems).
    out_text_sep = f"{out_text_block}\n\n" if out_text_block else ""
    return (
...
def _graph_py(spec: SystemSpec, state_name: str, deps_name: str) -> str:
    ...
    return (
        ...
        f"async def run_{spec.name}(text: str, *, model: str) -> {run_end_type}:\n"
