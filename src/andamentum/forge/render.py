...
def _graph_py(spec: SystemSpec, state_name: str, deps_name: str) -> str:
    ...
    # the graph's edge; every node stays pure (touches only State). Single-record, constant
    # key "_": the durable record's content seeds the entity's State field, and the final
    # value is saved back. No entity → these blocks are empty (stateless, unchanged).
    state_kwargs = [f"{primary}=data.{primary}"]
    load_block = ""
    save_block = ""
    for e in spec.entities:
        if not e.state_field:
            continue
        rec = f"_record_{e.state_field}"
        load_block += f'    {rec} = store_obj.get("{e.record_type}", "_")\n'
        state_kwargs.append(
            f'{e.state_field}=({rec}["content"] if {rec} is not None else "")'
        )
        save_block += (
            f'    store_obj.add("{e.record_type}", "_", '
            f'{{"id": "_", "content": state.{e.state_field}}})\n'
        )
    state_args = ", ".join(state_kwargs)
    ...
    return (
        ...
        f"    store_obj = Store(store)\n"
        f"{load_block}"
        f"    state = {state_name}({state_args})\n"
        f"    deps = {deps_name}(model=model, store=store_obj)\n"
...
def _smoke_test_py(spec: SystemSpec, state_name: str, deps_name: str) -> str:
    ...
    entity_seeds = "".join(
        f', {e.state_field}=""' for e in spec.entities if e.state_field
    )
