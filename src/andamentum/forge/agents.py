...
TYPE_NODE_PROMPT = (
    ...
    "(plus `input`, the graph input), in order, the LAST one being the most recent step's output. "
    "These are the ONLY things this step can read. Steps form a CHAIN: this step should almost "
    "always READ the output of the step right before it (the most recent variable) and build on "
    "it. Read `input` (the raw original text) ONLY if this step genuinely needs the unprocessed "
    "input — NEVER re-read `input` when an earlier step already turned it into something this step "
    "should refine. Then PRODUCE one new variable a later step will read. "
