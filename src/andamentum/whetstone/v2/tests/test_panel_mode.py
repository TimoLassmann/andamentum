...
from andamentum.whetstone.v2.agents import KeywordExtractionOutput
from andamentum.whetstone.v2.deps import ReviewDeps
from andamentum.whetstone.v2.nodes.expert_review import (
    ExpertReview as ExpertReviewNode,
)
from andamentum.whetstone.v2.nodes.extract_keywords import ExtractKeywords
from andamentum.whetstone.v2.nodes.generate_expert_panel import GenerateExpertPanel
from andamentum.whetstone.v2.nodes.panel_synthesise import PanelSynthesise
from andamentum.whetstone.v2.schemas import ReviewResult, SectionCard
...
class TestExtractKeywordsNode:
    async def test_extracts_disciplines_from_agent(self) -> None:
        ...
        with mock.patch(
            "andamentum.whetstone.v2.nodes.extract_keywords.build_pydantic_ai_agent",
            return_value=fake,
        ):
...
class TestGenerateExpertPanelNode:
    async def test_generates_n_profiles_in_parallel(self) -> None:
        ...
        with mock.patch(
            "andamentum.whetstone.v2.nodes.generate_expert_panel.build_pydantic_ai_agent",
...
class TestExpertReviewNode:
    async def test_runs_one_review_per_profile(self) -> None:
        ...
        with mock.patch(
            "andamentum.whetstone.v2.nodes.expert_review.build_pydantic_ai_agent",
            side_effect=fake_build,
        ):
            ctx = _FakeContext(state, deps)
...
class TestPanelSynthesiseNode:
    async def test_synthesises_from_reviews(self) -> None:
        ...
        with mock.patch(
            "andamentum.whetstone.v2.nodes.panel_synthesise.build_pydantic_ai_agent",
            return_value=fake,
        ):
            ctx = _FakeContext(state, deps)
...
async def test_e2e_panel_mode(panel_canned: dict[str, Any]) -> None:
    ...
    import andamentum.whetstone.v2.nodes.expert_review as er_mod
    import andamentum.whetstone.v2.nodes.extract_keywords as ek_mod
    import andamentum.whetstone.v2.nodes.generate_expert_panel as gep_mod
    import andamentum.whetstone.v2.nodes.panel_synthesise as ps_mod
