...
The canonical green state (post whetstone-v3 consolidation): **pyright 23 errors (pre-existing test-only typing noise — pydantic-graph generic variance in `test_topology.py`, `Decomposition` dict-form fixtures in several test files), ruff clean, pytest 2152 passing (2 skipped, 25 deselected)**. Don't claim completion until you've run these three and seen that state. (The pytest count grew from 2075 → 2152 net: whetstone v2 deletion removed ~380 v2-only tests, the v3 consolidation added ~460 new tests covering the new criterion taxonomy / tripwire wiring / criteria-input surface / editor port / novelty 3-node pipeline / CLI subcommands / panel mode.)
